...
│
├── Test Scripts
│   └── tests/                    # k6 and Playwright test files
│
├── AI Analysis
...
│
└── 📄 Browser Testing
    └── Dockerfile.playwright-worker  # Playwright test container
```
...
│
└── 📄 Browser Tests
    └── playwright_browser_test.py  # Playwright test script
```
...
<Accordion>
  ...
  <AccordionItem title="playwright_browser_test.py (Browser)">
    **Purpose**: Playwright front-end testing
...
└── 📁 Site-based Organization
    └── site_name/
        ...
        │   │   ├── playwright_results_*.json   # Raw browser data
...
<Accordion>
  <AccordionItem title="Raw Data Files">
    ...
    - **`playwright_results_*.json`**: Playwright browser metrics (100MB-1GB)
  </AccordionItem>
