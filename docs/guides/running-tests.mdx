...
Learn how to execute load tests with the Load Testing & Optimization Agent, including protocol testing, browser testing, and comprehensive analysis.
...
4. 🌐 Browser Testing (if enabled)
   ├── Run xk6-browser container with browser optimizations
   ├── Execute browser_load_test.js
...
<Accordion>
  ...
  <AccordionItem title="Browser Testing (xk6-browser)">
...
jobs:
  load-test:
    ...
    steps:
      ...
      - name: Run load test
        run: |
          python run_test.py configs/ci-test.yaml
